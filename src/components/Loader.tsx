...
const Loader = ({ onComplete }: LoaderProps) => {
  ...
  const updateTextColor = () => {
    ...
    const fillHeight = fillRef.current ? parseFloat(window.getComputedStyle(fillRef.current).height) : 0;
    ...
    if (fillHeight > 0 && centerY > (loaderHeight - fillHeight)) {
      ...
      const fillColor = window.getComputedStyle(fillRef.current!).backgroundColor;
